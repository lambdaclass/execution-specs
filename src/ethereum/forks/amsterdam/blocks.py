...
from .transactions.frame_transaction import FrameStatus, FrameTransaction
...
class FrameReceipt:
    ...
    logs: Tuple[Log, ...]
    """
    A tuple of logs generated by this frame. Emptied when the frame's
    atomic batch was unrolled or a failing post-transaction frame
    reverted the execution body.
    """
...
@slotted_freezable
@dataclass
class FrameTransactionReceipt:
    """
    Result of a frame transaction's execution. Frame transaction
    receipts are included in the receipts trie.

    Unlike `Receipt`, there is no transaction-level status and no
    bloom filter: outcomes are reported per frame, and the logs of all
    frames contribute to the block's log bloom in frame order.
    """

    cumulative_gas_used: Uint
    """
    Total gas used in the block up to and including this transaction.
    This is the gas used after refunds, paid by the payer.
    """

    payer: Address
    """
    The account that paid for the transaction's gas.
    """

    frame_receipts: Tuple[FrameReceipt, ...]
    """
    A tuple of per-frame receipts, in frame order.
    """


def encode_receipt(
    tx: Transaction, receipt: Receipt | FrameTransactionReceipt
) -> Bytes | Receipt:
    r"""
    ...
    - FrameTransaction receipts are prefixed with `b"\x06"`.
...
def decode_receipt(
    ...
    receipts.
    - Receipts prefixed with `b"\x06"` are decoded as FrameTransaction
    receipts.
    - LegacyTransaction receipts are returned as is.
    """
    if isinstance(receipt, Bytes):
        if receipt[0] == 6:
            return rlp.decode_to(FrameTransactionReceipt, receipt[1:])
